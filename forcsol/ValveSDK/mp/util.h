...
//#define STRING(offset) reinterpret_cast<const char *>(offset)//gpGlobals->pStringBase + (uintp)offset
inline const char* STRING(string_t offset) {
	return (*g_pGlobal)->pStringBase + offset;
}
...
typedef enum Hull{ point_hull = 0, human_hull = 1, large_hull = 2, head_hull = 3 };
