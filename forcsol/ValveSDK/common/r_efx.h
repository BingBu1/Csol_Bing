...
struct efx_api_s
{
	...
	void		( *R_BulletImpactParticles )	( float * pos );//+20
	void		( *R_EntityParticles )			( struct cl_entity_s *ent );
	void		( *R_Explosion )				( float *pos, int model, float scale, float framerate, int flags );
	void		(*unk5)							(int a, int b, int c, int d, int e, int f);
	void		( *R_FizzEffect )				( struct cl_entity_s *pent, int modelIndex, int density );//2C
	...
	void		(*unk4)(void);
	int			( *Draw_DecalIndex )			( int id );//Used decal
