...
{
	// sprite handlers
	HSPRITE						( *pfnSPR_Load )			( const char *szPicName );//0
	...
	void                     ( *nop )	( int x, int y, int width, int height );//CSOL多出来的一个不知道什么用
	void						( *pfnSPR_DisableScissor )	( void );//10
	...
	int							( *pfnServerCmd )			( char *szCmdString );//20
	...
	void						( *pfnDrawConsoleStringLen )(  const char *string, int *length, int *height );//30
...
// Added for user input processing
	...
	void						( *GetMousePosition )		( int *mx, int *my );//50
	...
	struct pmtrace_s			*( *PM_TraceLine )			( float *start, float *end, int flags, int usehull, int ignore_pe );//60
	...
	void						( *pfnHookEvent )			( char *name, void ( *pfnEvent )( struct event_args_s *args ) );//70
	int							(*Con_IsVisible)			();
	...
	::byte*						(*COM_LoadFile)				( char *path, int usehunk, int *pLength );//80
	...
	struct model_s				*( *LoadMapSprite )			( const char *filename );//90
	...
	void						( *pfnSetMousePos )( int x, int y );//100
	...
	void ( *SetLightmapColor )( float r, float g, float b );//110
	...
	int ( *pfnDrawLocalizedConsoleString )( int x, int y, const char* str, int r, int g, int b);
	...
	int ( *pfnCheckSoundFile )( const char* path );//120
	...
	void ( *pfnPlaySound2 )( const char* sound, float vol, float pitch );//130
	...
	void (*Spr_DrawHolesScaled)(char a1, int a2, int a3, int a4, DWORD* a5, int a6, int a7, int a8);
	int (*SPR_Replace)(int a1, int a2);
	int (*SPR_DrawAdditiveScaled)(int a1, int a2, int a3, DWORD* a4, int a5, int a6, int a7);
	int (*LeaveGameRoom)(int 不知);
	int (*Unk1)(int dontno);
	int	(*验证某种东西不清楚)();
	PDWORD(*GetCServerChannelManager)(void); // +init;//140
	int (*LoadSound) (char* 音效路径, float 未知, int 未知2);
	DWORD* (*GetInventory)();
	DWORD* (*GetIGClassMenuMgr)();
	DWORD* (*GetIGBuyMenuMgr)();
	DWORD* (*GetChattingManager)();
	DWORD* (*某个结构体的指针大概1)();
	DWORD* (*某个结构体的指针大概2)();
	DWORD* (*某个结构体的指针大概3)();
	DWORD* (*关于ITEM的指针)();
	DWORD* (*某个结构体的指针大概4)();//150
	DWORD* (*某个结构体的指针大概5)();//GetSharedDataMgr
	DWORD* (*某个结构体的指针大概6)();//GetCrossHairMgr
	DWORD* (*某个结构体的指针大概7)();
	DWORD* (*某个结构体的指针大概8)();
	DWORD* (*某个结构体的指针大概9)();
	DWORD* (*某个结构体的指针大概10)();
	int* (不清楚)(::byte* al, int a2);
	DWORD* (*关于location的指针)();
	DWORD* (*某个结构体的指针大概11)();
	DWORD* (*某个结构体的指针大概12)();//160
	DWORD* (*某个结构体的指针大概13)();
	DWORD* (*某个结构体的指针大概14)();
	DWORD* (*某个结构体的指针大概15)();
	int (*不清楚的)(int a1);
	DWORD* (*某个结构体的指针大概16)();
	int (*Pbuf_AddText)(int a1);
	DWORD* (*某个结构体的指针大概17)();
	cvar_s* (Cvar_RegisterVariable)(char *指令名称,int *不清楚,int flags);
	DWORD* (*某个结构体的指针大概18)();
	DWORD* (*某个结构体的指针大概19)();//170
	void (*Hud_NoticePrint)(const char* 文本, int corlor);
	int (*不清楚的2)(int a1, int a2, int a3, int a4, int a5);
	DWORD* (*某个结构体的指针大概20)();
	DWORD* (*某个结构体的指针大概21)();
	DWORD* (*某个结构体的指针大概22)();
	DWORD* (*某个结构体的指针大概23)();
	...
	int(*HUD_DrawNormalTriangles)       ();//240
